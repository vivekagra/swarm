...
# define trigpin_back_sonar A0
# define echopin_back_sonar A1

# define trigpin_left_sonar A2
# define echopin_left_sonar A3

# define trigpin_right_sonar A4
# define echopin_right_sonar A5
...
std_msgs::Float32 dist_back_msg;
std_msgs::Float32 dist_left_msg;
std_msgs::Float32 dist_right_msg;
...
ros::Publisher dist_back("dist_back",  &dist_back_msg)
ros::Publisher dist_left("dist_left",  &dist_left_msg)
ros::Publisher dist_right("dist_right",&dist_right_msg)
...
void obstacle()
{
	digitalWrite(trigpin_back_sonar,Low);
	digitalWrite(trigpin_left_sonar,Low);
	digitalWrite(trigpin_right_sonar,Low);
	delayMicroseconds(2);
	digitalWrite(trigpin_back_sonar,HIGH);
	digitalWrite(trigpin_left_sonar,HIGH);
	digitalWrite(trigpin_right_sonar,HIGH);
	delayMicroseconds(10);
	digitalWrite(trigpin_back_sonar,Low);
	digitalWrite(trigpin_left_sonar,Low);
	digitalWrite(trigpin_right_sonar,Low);
	duration_back = pulseIn(echopin_back_sonar, )
}
...
void setup()
{
	...
	pinMode(trigpin_back_sonar,OUTPUT);
	pinMode(echopin_back_sonar,OUTPUT);
	pinMode(trigpin_back_sonar,OUTPUT);
	pinMode(echopin_back_sonar,OUTPUT);
	pinMode(trigpin_back_sonar,OUTPUT);
	pinMode(echopin_back_sonar,OUTPUT);
